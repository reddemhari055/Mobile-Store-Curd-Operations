...
async function loadMobiles() {
    ...
    mobileList.innerHTML = data.map((m, index) => `
        <tr class="${index % 2 !== 0 ? 'bg-gray' : ''}">
            ...
            <td>${m.storage}</td>
            <td>
                <button class="btn-edit" onclick='openEditModal(${JSON.stringify(m)})'>
                    Edit
                </button>
            </td>
            <td>
                <button class="btn-delete" onclick="deleteMobile('${m._id}')">
                    Delete
                </button>
            </td>
        </tr>
...
function openEditModal(m) {
    ...
    document.getElementById('mobileId').value = m._id;
